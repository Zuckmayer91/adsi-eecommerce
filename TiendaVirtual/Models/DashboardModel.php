<?php 
	class DashboardModel extends Mysql
	{
		...
		public function lastProductosV(){
		    $sql = "SELECT p.IdProducto, p.Prod_Codigo, p.Prod_Precio, SUM(`DetP_Cantidad`) as Venta 
			FROM TblProducto p 
			INNER JOIN TblDetalle_pedido d 
			ON p.IdProducto = d.Prod_IdProducto
			INNER JOIN TblPedido pd 
			ON pd.IdPedido = d.Ped_IdPedido 
			WHERE pd.Ped_Proceso = 'Completo' 
			GROUP by p.IdProducto DESC LIMIT 10;";
			$request = $this->select_all($sql);
			return $request;
		}	
