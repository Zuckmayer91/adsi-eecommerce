<?php 
	class PedidosModel extends Mysql
	{
		...
		public function reembolsoPaypal(string $idtransaccion, string $observacion){
			...
			if(!empty($requestData)){
				...
				if(isset($objTransaccion->status) and  $objTransaccion->status == "COMPLETED"){
					...
					if($request_insert > 0){
	        			$updatePedido  = "UPDATE TblPedido SET Ped_Proceso = ? WHERE IdPedido = $idpedido";
		...
		public function updatePedido(int $idpedido, $transaccion = NULL, $idtipopago = NULL, string $estado){
			if($transaccion == NULL){
				$query_insert  = "UPDATE TblPedido SET Ped_Proceso = ?  WHERE IdPedido = $idpedido ";
	        	$arrData = array($estado);
			}else{
				$query_insert  = "UPDATE TblPedido SET Ped_Referenciacobro = ?, Tpag_IdTipopago = ?,Ped_Proceso = ? WHERE IdPedido = $idpedido";
